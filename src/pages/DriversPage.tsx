...
interface DriverRoster {
  id: string;
  name: string;
  cardNumber: string; // Used as Tachograph No (EU) or CDL/License No (US)
  cardExpiry: string;
...
export function DriversPage({ drivers: parentDrivers, onAddDriver: parentAddDriver }: DriversPageProps) {
  ...
  const isEU = region?.includes('EU') || localStorage.getItem('regionMode')?.includes('EU');
  ...
  const [rosterDrivers, setRosterDrivers] = useState<DriverRoster[]>([
    isEU
      ...
      : {
          id: 'DRV-201',
          name: 'John Doe',
          cardNumber: 'DL-9988231US',
          cardExpiry: '2029-03-15',
          weeklyRestStatus: 'Compliant',
          hoursThisWeek: 34.0,
          lastBorderCrossing: 'Class A (Hazmat)'
        }
  ]);
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const newRosterDriver: DriverRoster = {
      ...
      weeklyRestStatus: 'Compliant',
      hoursThisWeek: 0,
      lastBorderCrossing: isEU ? 'None' : 'Class A (Standard)'
    };
  ...
  const getStatusBadge = (status: DriverRoster['weeklyRestStatus']) => {
    switch (status) {
      ...
      case 'Warning':
        return <span style={{ background: 'rgba(245, 158, 11, 0.15)', color: '#f59e0b', border: '1px solid rgba(245, 158, 11, 0.3)', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600 }}>Near Limit</span>;
      case 'Rest Required':
        return <span style={{ background: 'rgba(239, 68, 68, 0.15)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.3)', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600 }}>Rest Due</span>;
    }
  };
    ...
    <div style={{ color: '#f8fafc', padding: '10px' }}>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: '600', marginBottom: '4px' }}>
          {isEU ? '🇪🇺 Drivers & Driver Card Roster' : '🇺🇸 US Drivers & CDL License Roster'}
        </h2>
        <p style={{ color: '#94a3b8', margin: 0, fontSize: '0.85rem' }}>
          {isEU
            ? 'Monitor EU Smart Tachograph driver card validity, 56-hour fortnightly drive limits, and weekly rest periods.'
            : 'Monitor US driver license numbers, CDL classifications, and FMCSA hours-of-service compliance.'}
        </p>
      </div>

      <div style={{ background: '#0b1329', border: '1px solid #1e293b', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
        <h3 style={{ margin: '0 0 14px 0', color: '#38bdf8', fontSize: '1rem' }}>
          {isEU ? 'Register Driver Card' : 'Register US Driver & License'}
        </h3>
        <form onSubmit={handleFormSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
          ...
          <div>
            <label style={{ color: '#94a3b8', fontSize: '0.75rem', display: 'block', marginBottom: '4px' }}>
              {isEU ? 'Tachograph Card No.' : 'US Driver License / CDL No.'}
            </label>
            <input
              type="text"
              placeholder={isEU ? 'e.g. E5544332210000' : 'e.g. DL-12345678'}
              value={cardNumber}
          ...
          <div>
            <label style={{ color: '#94a3b8', fontSize: '0.75rem', display: 'block', marginBottom: '4px' }}>Expiry Date</label>
            <input
              type="date"
              value={cardExpiry}
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            ...
            >
              {isEU ? 'Add Driver Card' : 'Add US Driver'}
            </button>
          </div>
        </form>
      </div>
      ...
      <div style={{ background: '#0b1329', border: '1px solid #1e293b', borderRadius: '12px', padding: '20px' }}>
        <h3 style={{ margin: '0 0 16px 0', color: '#f8fafc', fontSize: '1rem' }}>
          {isEU ? 'Active Driver Card Compliance Roster' : 'Active US Driver Roster'}
        </h3>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
            <thead>
              <tr style={{ background: '#020617', color: '#94a3b8' }}>
                ...
                <th style={{ padding: '10px' }}>{isEU ? 'Card Number' : 'License / CDL No.'}</th>
                <th style={{ padding: '10px' }}>Expiry</th>
                <th style={{ padding: '10px' }}>{isEU ? 'Weekly Driving' : 'HOS Limit'}</th>
                <th style={{ padding: '10px' }}>Compliance</th>
                <th style={{ padding: '10px' }}>{isEU ? 'Recent Border Cross' : 'Endorsements'}</th>
              </tr>
            </thead>
            <tbody>
              {rosterDrivers.map((d) => (
                <tr key={d.id} style={{ borderBottom: '1px solid #1e293b' }}>
                  ...
                  <td style={{ padding: '10px', fontWeight: 'bold', color: d.hoursThisWeek >= 50 ? '#ef4444' : '#fff' }}>
                    {d.hoursThisWeek} {isEU ? '/ 56 hrs' : '/ 70 hrs'}
                  </td>
